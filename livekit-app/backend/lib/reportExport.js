  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        ...
        Author: 'Lalia',
        Creator: 'Lalia — multilingual meetings',
      },
    });
    const chunks = [];
    ...
    for (let i = range.start; i < range.start + range.count; i += 1) {
      ...
      doc
        ...
        .fillColor(COLORS.muted)
        .text(
          `Generated by Lalia · ${created.toLocaleDateString('en-US', { dateStyle: 'medium' })} · Page ${i + 1} of ${range.count}`,
...
function reportToMarkdown({ report, meetingTitle, orgName }) {
  ...
  const header = [
    ...
    '',
    `> Generated by Lalia on ${created.toLocaleString('en-US', { dateStyle: 'medium', timeStyle: 'short' })}` +
