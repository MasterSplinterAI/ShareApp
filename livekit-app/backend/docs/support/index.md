# Parley Support Knowledge Base

Internal corpus for AI triage and support replies. Update when product behavior changes.

## Categories

- **Customer support** — how-to, billing questions, account issues
- **Bug reports** — reproducible defects; escalate to engineering via GitHub issue after approval
- **Feature requests** — product ideas; backlog via GitHub enhancement issues

## Key product areas

| Topic | Doc |
|-------|-----|
| Meetings & rooms | product/meetings.md |
| Live translation | product/translation.md |
| Video effects / backgrounds | product/video-effects.md |
| Billing & plans | product/billing.md |
| Password & account | product/account-settings.md |
| Transcripts & security | product/transcripts-security.md |
| Common questions | faq.md |
| Audio/video issues | troubleshooting/audio-video.md |
| Translation stuck | troubleshooting/translation-stuck.md |
| Mobile effects delay | troubleshooting/effects-mobile.md |

## Escalation

See runbooks/escalation.md for billing disputes, abuse, and legal requests — always route to human review.
